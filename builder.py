...
BuildMode = Literal["base", "fullmeta"]
...
def _init_db(conn: sqlite3.Connection, mode: BuildMode) -> None:
    cur = conn.cursor()
...
def _build_db(
    ...
    try:
        ...
        raise RuntimeError(
            "lxml is required for building from dblp.xml. "
            "Install it in the runtime environment."
        ) from exc

    log(f"Building sqlite db from {xml_path} -> {db_path} (mode={mode})")
    ...
    if mode == "fullmeta":
        ...
        insert_pub = "INSERT INTO publications(title) VALUES (?);"
...
def run_pipeline(
    ...
    log(f"Pipeline start (mode={config.mode}, data_dir={config.data_dir})")
